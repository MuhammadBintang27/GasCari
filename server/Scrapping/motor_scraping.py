...
def process_url(url):
    ...
    try:
        ...
        motor_collection.insert_one(article_data)
...
motor_collection = db_scrapping['motor']
all_collection = db_scrapping['all']

# Membaca URL dari MongoDB
urls = (
    [doc['link'] for doc in collection_otodetik.find({'category': 'motor'})] +
    [doc['link'] for doc in collection_autonetmagz.find({'category': 'motor'})]
)
urldoc = {}
skipped_urls = []
...
# Update JSON file
update_json(os.path.join('urldoc', 'motor_urldoc.json'), urldoc)
